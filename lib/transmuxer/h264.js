...
shaka.transmuxer.H264 = class {
  ...
  static parseInfo(nalus) {
    ...
    let spsNalu = nalus.find((nalu) => {
      return nalu.type == H264.NALU_TYPE_SPS_;
    });
    let ppsNalu = nalus.find((nalu) => {
      return nalu.type == H264.NALU_TYPE_PPS_;
    });

    if (!spsNalu && shaka.transmuxer.H264.lastSpsNalu) {
      spsNalu = shaka.transmuxer.H264.lastSpsNalu;
    } else {
      shaka.transmuxer.H264.lastSpsNalu = spsNalu;
    }
    if (!ppsNalu && shaka.transmuxer.H264.lastPpsNalu) {
      ppsNalu = shaka.transmuxer.H264.lastPpsNalu;
    } else {
      shaka.transmuxer.H264.lastPpsNalu = ppsNalu;
    }
    if (!spsNalu || !ppsNalu) {
      return null;
    }
...
shaka.transmuxer.H264.lastSpsNalu = null;
shaka.transmuxer.H264.lastPpsNalu = null;
/**
